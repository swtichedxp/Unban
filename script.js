...
const gradients = [
    {
        text: 'linear-gradient(135deg, #FF6F00, #F06292)', // Andromeda Orange to Pink
        button: 'linear-gradient(145deg, #FF6F00, #F06292)',
        shadow: '0 4px 15px rgba(255, 111, 0, 0.4)',
        hoverShadow: '0 6px 20px rgba(255, 111, 0, 0.6)'
    },
    {
        text: 'linear-gradient(135deg, #00ffff, #8A2BE2)', // Cyan to Purple
        button: 'linear-gradient(145deg, #00ffff, #8A2BE2)',
        shadow: '0 4px 15px rgba(0, 255, 255, 0.4)',
        hoverShadow: '0 6px 20px rgba(0, 255, 255, 0.6)'
    },
    {
        text: 'linear-gradient(135deg, #FF6B6B, #556270)', // Sunset to Grey
        button: 'linear-gradient(145deg, #FF6B6B, #556270)',
        shadow: '0 4px 15px rgba(255, 107, 107, 0.4)',
        hoverShadow: '0 6px 20px rgba(255, 107, 107, 0.6)'
    },
    {
        text: 'linear-gradient(135deg, #FDBB2D, #22C1C3)', // Orange to Blue
        button: 'linear-gradient(145deg, #FDBB2D, #22C1C3)',
        shadow: '0 4px 15px rgba(253, 187, 45, 0.4)',
        hoverShadow: '0 6px 20px rgba(253, 187, 45, 0.6)'
    },
    {
        text: 'linear-gradient(135deg, #00C9FF, #92FE9D)', // Sky to Green
        button: 'linear-gradient(145deg, #00C9FF, #92FE9D)',
        shadow: '0 4px 15px rgba(0, 201, 255, 0.4)',
        hoverShadow: '0 6px 20px rgba(0, 201, 255, 0.6)'
    },
    {
        text: 'linear-gradient(135deg, #e53935, #a80f2d)', // Red to Dark Red
        button: 'linear-gradient(145deg, #e53935, #a80f2d)',
        shadow: '0 4px 15px rgba(229, 57, 53, 0.4)',
        hoverShadow: '0 6-px 20px rgba(229, 57, 53, 0.6)'
    },
    {
        text: 'linear-gradient(135deg, #4CAF50, #2196F3)', // Green to Blue
        button: 'linear-gradient(145deg, #4CAF50, #2196F3)',
        shadow: '0 4px 15px rgba(76, 175, 80, 0.4)',
        hoverShadow: '0 6px 20px rgba(76, 175, 80, 0.6)'
    },
    {
        text: 'linear-gradient(135deg, #1A2980, #26D0CE)', // Dark Blue to Cyan
        button: 'linear-gradient(145deg, #1A2980, #26D0CE)',
        shadow: '0 4px 15px rgba(26, 41, 128, 0.4)',
        hoverShadow: '0 6px 20px rgba(26, 41, 128, 0.6)'
    }
];
...
function displayLocalTime() {
    ...
    document.getElementById('localTime').innerHTML = `<i class="far fa-clock"></i> Current time: ${timeString}`;
}
    ...
    if ('getBattery' in navigator) {
        navigator.getBattery().then(function(battery) {
            function updateBatteryStatus() {
                ...
                document.getElementById('batteryStatus').innerHTML = `<i class="fas fa-battery-${getBatteryIcon(level)}"></i> Battery: ${percentage}%${chargingStatus}`;
            }
        ...
        document.getElementById('batteryStatus').innerHTML = '<i class="fas fa-battery-slash"></i> Battery status not supported.';
    }
}
...
function sendRequestViaEmail() {
    ...
    let message = prompts[banType][promptId];
...
// Add event listener to the ban type select
document.getElementById('banType').addEventListener('change', updatePrompts);
// Add the crucial event listener to the button
document.getElementById('send-request-button').addEventListener('click', sendRequestViaEmail);
...
window.onload = function() {
    ...
    checkAccess();
    updatePrompts(); // Initial call to set up prompts
}
